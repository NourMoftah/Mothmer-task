...
export function localizeApiData<T>(value: T, locale?: ApiLocale): T {
  ...
  if (Array.isArray(value)) return value.map((item) => localizeApiData(item, locale)) as T;
  if (typeof value === "object") {
    return Object.fromEntries(Object.entries(value as Record<string, unknown>).map(([key, item]) => [key, localizeApiData(item, locale)])) as T;
  }
  return value;
}
